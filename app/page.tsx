...
import { Book, X, Star, Share2, Loader2, Trash2, Headphones, Sparkles, Droplets, Wind } from 'lucide-react';
...
export default function BambooForest() {
  ...
  return (
    <main 
      ...
      onPointerDown={handleGlobalClick} // [Update] 터치 시 물결 생성
    >
      ...
      <motion.div className="absolute inset-0 w-full h-full" animate={{ filter: hasStarted ? 'blur(0px)' : 'blur(20px)', opacity: hasStarted ? 1 : 0 }} transition={{ duration: 2 }}>
        ...
        <motion.div className="absolute top-20 left-1/2 -translate-x-1/2 pointer-events-none z-0 mix-blend-screen" style={{ x: moonX, y: moonY }}>
            <div className="relative w-32 h-32 opacity-80">
                <svg viewBox="0 0 24 24" className="w-full h-full text-yellow-100 blur-[0.5px] drop-shadow-[0_0_15px_rgba(255,255,200,0.5)]"><path d={moonPath} fill="currentColor" /></svg>
                <div className="absolute inset-0 bg-yellow-100/20 blur-[50px] rounded-full" />
            </div>
        </motion.div>
            ...
            <AnimatePresence>
                {ripples.map((ripple) => (
                    <motion.div
                        ...
                        className="absolute border border-white/30 rounded-full bg-white/5 backdrop-blur-[1px]"
                        style={{
                            left: ripple.x,
                            top: ripple.y,
                            width: 100,
        ...
        <AnimatePresence>
          {!hasCollectedDew && callStatus === 'idle' && !showJournal && !isBreathing && (
            <motion.button initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 2, filter: "blur(10px)" }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={collectDew} className="absolute top-8 right-8 z-50 group cursor-pointer flex flex-col items-center gap-2">
              <div className="relative flex items-center justify-center w-14 h-14 bg-black/20 backdrop-blur-md rounded-full border border-white/10 shadow-lg group-hover:bg-white/10 transition-colors">
                <Droplets className="text-blue-200 drop-shadow-[0_0_10px_rgba(100,200,255,0.8)]" size={24} strokeWidth={1.5} />
                <div className="absolute inset-0 bg-blue-400 rounded-full opacity-20 animate-ping" />
              </div>
              <span className="text-[10px] text-blue-100/80 tracking-widest uppercase font-medium bg-black/40 backdrop-blur-sm px-2 py-1 rounded-full border border-white/5 opacity-0 group-hover:opacity-100 transition-opacity">Collect Dew</span>
            </motion.button>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {dailyQuote && (
            <motion.div initial={{ opacity: 0, y: -20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.9 }} className="absolute top-28 right-8 z-50 w-[280px] md:w-[320px] pointer-events-none">
              <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-6 rounded-2xl shadow-2xl relative overflow-hidden">
                <div className="absolute -top-10 -right-10 w-20 h-20 bg-blue-500/20 blur-2xl rounded-full" />
                <p className="text-white/90 text-sm md:text-base font-light italic leading-relaxed relative z-10">"{dailyQuote}"</p>
                <div className="mt-4 flex justify-between items-center border-t border-white/10 pt-3"><span className="text-blue-200/60 text-[10px] uppercase tracking-widest flex items-center gap-1"><Sparkles size={10} /> Daily Wisdom</span><span className="text-yellow-200/80 text-[10px] font-mono font-bold">+50 XP</span></div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
                ...
                {selectedMemory && (
                  <motion.div key="memory-card" id={`memory-card-${selectedMemory.id}`} drag="y" dragConstraints={{ top: 0, bottom: 0 }} dragElastic={{ top: 0.05, bottom: 0.5 }} onDragEnd={handleDragEnd} initial={{ y: 50, opacity: 0, scale: 0.9 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 100, opacity: 0, scale: 0.9, transition: { duration: 0.2 } }} whileDrag={{ scale: 0.98 }} className="absolute bottom-32 left-0 right-0 mx-auto w-[90%] md:w-[400px] bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-2xl border border-white/20 p-8 rounded-3xl shadow-2xl overflow-hidden cursor-grab active:cursor-grabbing">
                    ...
                    <div className="absolute bottom-4 right-4 flex gap-3">
                      <motion.button onClick={() => deleteMemory(selectedMemory.id)} disabled={isDeleting === selectedMemory.id} className="p-2 bg-red-500/10 rounded-full hover:bg-red-500/20 text-red-200/70 hover:text-red-200 transition-all disabled:opacity-50" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>{isDeleting === selectedMemory.id ? <Loader2 size={20} className="animate-spin" /> : <Trash2 size={20} />}</motion.button>
                      <motion.button onClick={() => shareMemory(selectedMemory)} disabled={capturingId === selectedMemory.id} className="p-2 bg-white/10 rounded-full hover:bg-white/20 text-white/70 hover:text-white transition-all disabled:opacity-50" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>{capturingId === selectedMemory.id ? <Loader2 size={20} className="animate-spin" /> : <Share2 size={20} />}</motion.button>
                    </div>
        ...
        <motion.div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ x: spiritX, y: spiritY }}>
           <motion.div
            ...
            animate={ isBreathing ? { scale: [1, 1.15, 1.15, 1], filter: ['brightness(1)', 'brightness(1.3) drop-shadow(0 0 30px rgba(255,255,255,0.6))', 'brightness(1.3) drop-shadow(0 0 30px rgba(255,255,255,0.6))', 'brightness(1)'], } : showJournal ? { filter: 'blur(20px) brightness(0.3)', opacity: 0.5 } : callStatus === 'speaking' ? { y: 0, filter: 'brightness(1.2) drop-shadow(0 0 20px rgba(255,255,255,0.4))' } : callStatus === 'listening' ? { scale: 1.05, y: 0, filter: 'brightness(1.1)' } : callStatus === 'idle' ? { scale: 1, filter: 'blur(10px) grayscale(100%) opacity(0.7)', y: 0 } : { scale: 1, filter: 'brightness(1) drop-shadow(0 0 0px rgba(0,0,0,0))', y: [0, -10, 0] } } transition={ isBreathing ? { duration: 19, times: [0, 0.21, 0.58, 1], repeat: Infinity, ease: "easeInOut" } : { default: { duration: 0.5 }, y: { duration: 3, repeat: Infinity, ease: "easeInOut" } } }>
        ...
        {/* Soul Level */}
        <div className="absolute bottom-8 left-8 z-50 flex flex-col gap-1 items-start">
