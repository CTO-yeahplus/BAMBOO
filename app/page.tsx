...
// [Fix] React default import 추가 (React.KeyboardEvent 사용을 위해)
import React, { useState, useMemo, useEffect } from 'react';
import Image from 'next/image';
import { motion, AnimatePresence, useSpring, useMotionValue, PanInfo, useTransform } from 'framer-motion';
import { Book, X, Star, Share2, Loader2, Trash2, Headphones, Sparkles, Droplets, Wind, Trees, CloudRain, Flame, Waves, Lock, Sun, Settings2, Volume2, Mic, LogIn, LogOut, Hourglass, Send, Clock, LayoutGrid, MousePointerClick, ArrowUp, Keyboard, SendHorizontal, Palette, Gem, Check } from 'lucide-react';import { useBambooEngine } from './hooks/useBambooEngine';
...
const GoldenCocoon = ({ isLocked }: { isLocked: boolean }) => (
  <div className="relative group">
    <div className="absolute inset-0 bg-yellow-500/30 blur-xl rounded-full animate-pulse" />
    {isLocked ? (
      <svg viewBox="0 0 24 24" className="w-10 h-10 drop-shadow-[0_0_15px_rgba(234,179,8,0.6)]" fill="none">
        <line x1="12" y1="0" x2="12" y2="4" stroke="rgba(255,255,255,0.3)" strokeWidth="1" />
        <path d="M12 4C9 4 7 8 7 13C7 18 9 22 12 22C15 22 17 18 17 13C17 8 15 4 12 4Z" className="fill-yellow-600/80 stroke-yellow-200" strokeWidth="1.5" />
        <path d="M8 10C9 11 11 11.5 12 11C13 10.5 15 11 16 12" stroke="rgba(255,255,255,0.4)" strokeLinecap="round" />
        <path d="M8 14C9 15 11 15.5 12 15C13 14.5 15 15 16 16" stroke="rgba(255,255,255,0.4)" strokeLinecap="round" />
      </svg>
    ) : (
      <svg viewBox="0 0 24 24" className="w-12 h-12 drop-shadow-[0_0_20px_rgba(250,204,21,0.8)] animate-[bounce_3s_infinite]">
        <path d="M12 12C12 12 8 6 4 8C0 10 2 16 6 16C8 16 11 14 12 12Z" className="fill-yellow-300 opacity-90" />
        <path d="M12 12C12 12 16 6 20 8C24 10 22 16 18 16C16 16 13 14 12 12Z" className="fill-yellow-300 opacity-90" />
        <path d="M12 12C12 12 10 18 8 20C6 22 4 20 6 18C8 16 11 14 12 12Z" className="fill-yellow-500 opacity-80" />
        <path d="M12 12C12 12 14 18 16 20C18 22 20 20 18 18C16 16 13 14 12 12Z" className="fill-yellow-500 opacity-80" />
      </svg>
    )}
  </div>
);
...
const ConstellationLayer = ({ memories }: { memories: Memory[] }) => {
  ...
  const getEmotionColor = (emotion?: string) => {
    switch (emotion) {
      ...
      case 'sadness': return '#60A5FA';
      case 'loneliness': return '#9CA3AF';
    ...
    <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
      <defs>
        {sortedMemories.map((memory, index) => {
          ...
          const prev = sortedMemories[index - 1] as VisualMemory | undefined;
          const curr = memory as VisualMemory;
          if (!prev) return null;
          const id = `grad-${prev.id}-${curr.id}`;
          return (
            <linearGradient key={id} id={id} x1={`${prev.x}%`} y1={`${prev.y}%`} x2={`${curr.x}%`} y2={`${curr.y}%`} gradientUnits="userSpaceOnUse">
              <stop offset="0%" stopColor={getEmotionColor(prev.emotion)} stopOpacity="0.4" />
              <stop offset="100%" stopColor={getEmotionColor(curr.emotion)} stopOpacity="0.4" />
            </linearGradient>
          );
        })}
      </defs>
      {sortedMemories.map((memory, index) => {
        ...
        const curr = memory as VisualMemory;
        if (!prev) return null;
        const gradId = `grad-${prev.id}-${curr.id}`;
        const isClient = typeof window !== 'undefined';
        const startX = isClient ? prev.x * window.innerWidth / 100 : 0;
        const startY = isClient ? prev.y * window.innerHeight / 100 : 0;
        const endX = isClient ? curr.x * window.innerWidth / 100 : 0;
        const endY = isClient ? curr.y * window.innerHeight / 100 : 0;
        return (
          <motion.g key={`connection-${curr.id}`}>
            <motion.line x1={`${prev.x}%`} y1={`${prev.y}%`} x2={`${curr.x}%`} y2={`${curr.y}%`} stroke={`url(#${gradId})`} strokeWidth="1.5" initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: 1, opacity: 1 }} transition={{ duration: 2, delay: index * 0.3, ease: "easeInOut" }} />
            {isClient && (<motion.circle r="1" fill="white" initial={{ offsetDistance: "0%" }} animate={{ offsetDistance: "100%" }}><animateMotion dur={`${3 + Math.random() * 2}s`} repeatCount="indefinite" path={`M${startX},${startY} L${endX},${endY}`} /></motion.circle>)}
          </motion.g>
        );
      })}
    </svg>
...
const OrbitLayer = ({ memories, onSelect }: { memories: Memory[], onSelect: (m: Memory) => void }) => {
  ...
  const orbitPoints = useMemo(() => {
    return Array.from({ length: daysInMonth }, (_, i) => {
      ...
      const memory = memories.find(m => {
        ...
        return mDate.getDate() === day && mDate.getMonth() === today.getMonth();
      });
  ...
  const getGlowColor = (emotion?: string) => {
    switch (emotion) {
      case 'anger': return 'text-red-400 drop-shadow-[0_0_15px_rgba(248,113,113,0.8)]';
      case 'sadness': return 'text-blue-400 drop-shadow-[0_0_15px_rgba(96,165,250,0.8)]';
      case 'loneliness': return 'text-gray-300 drop-shadow-[0_0_15px_rgba(209,213,219,0.8)]';
      case 'happy': return 'text-yellow-300 drop-shadow-[0_0_15px_rgba(253,224,71,0.8)]';
  ...
  return (
    <div className="absolute inset-0 w-full h-full pointer-events-none">
      <svg className="absolute inset-0 w-full h-full opacity-10" viewBox="0 0 100 100" preserveAspectRatio="none">
        <path d={orbitPoints.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`).join(' ')} fill="none" stroke="white" strokeWidth="0.2" strokeDasharray="1,1" />
      </svg>
      {orbitPoints.map((point, index) => (
        <motion.div key={point.day} className="absolute flex flex-col items-center justify-center -translate-x-1/2 -translate-y-1/2 pointer-events-auto" style={{ left: `${point.x}%`, top: `${point.y}%` }} initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: index * 0.03, type: "spring" }}>
          <button onClick={() => point.memory && onSelect(point.memory)} disabled={!point.memory} className={`group relative p-2 transition-transform hover:scale-125 ${!point.memory ? 'cursor-default' : 'cursor-pointer'}`}>
            <svg viewBox="0 0 24 24" className={`w-4 h-4 md:w-6 md:h-6 ${point.memory ? getGlowColor(point.memory.emotion) : 'text-white/5'}`}><path d={point.moonPath} fill="currentColor" /></svg>
            <span className="absolute -bottom-4 left-1/2 -translate-x-1/2 text-[8px] text-white/30 font-mono opacity-0 group-hover:opacity-100 transition-opacity">{point.day}</span>
            {point.memory && (<div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-max max-w-[120px] bg-black/80 backdrop-blur px-2 py-1 rounded border border-white/10 text-[8px] text-white/80 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none truncate z-50">{point.memory.summary}</div>)}
          </button>
        </motion.div>
      ))}
    </div>
...
// [New] Spirit Visual Components (for Customization)
          ...
          return (
              <div className="absolute inset-0 pointer-events-none">
                  {Array.from({ length: 5 }).map((_, i) => (
                      <motion.div key={i} className="absolute w-1 h-1 bg-yellow-200 rounded-full blur-[1px] shadow-[0_0_5px_yellow]" animate={{ x: [Math.random() * 200 - 100, Math.random() * 200 - 100], y: [Math.random() * 200 - 100, Math.random() * 200 - 100], opacity: [0, 1, 0] }} transition={{ duration: 3 + Math.random() * 2, repeat: Infinity, ease: "easeInOut" }} style={{ left: '50%', top: '50%' }} />
                  ))}
              </div>
          );
...
const SpiritAccessory = ({ type }: { type: string | null }) => {
  ...
  switch (type) {
      case 'head_flower':
          return <div className="absolute top-[15%] left-1/2 -translate-x-1/2 w-[60%] h-10 pointer-events-none opacity-80"><div className="flex justify-center gap-1"><span className="text-2xl drop-shadow-lg filter hue-rotate-15">🌸</span><span className="text-xl drop-shadow-lg mt-2">🌼</span><span className="text-2xl drop-shadow-lg filter -hue-rotate-15">🌸</span></div></div>;
      case 'head_fox':
          return <div className="absolute top-[20%] left-1/2 -translate-x-1/2 w-16 h-16 bg-white rounded-full shadow-lg flex items-center justify-center pointer-events-none border border-red-500/30"><div className="absolute top-4 left-3 w-3 h-1 bg-red-500 rounded-full rotate-12" /><div className="absolute top-4 right-3 w-3 h-1 bg-red-500 rounded-full -rotate-12" /><span className="text-3xl mt-1">🦊</span></div>;
      default: return null;
  }
};
...
export default function BambooForest() {
  const {
    ...
    isSilentMode, toggleSilentMode, sendTextMessage, // [Check] Ensure these are destructured
  ...
  // [Fix] Keyboard 'Enter' key with correct type
  ...
  const handleSelectMemoryWrapper = (m: Memory) => {
      const visualMem: VisualMemory = { ...m, x: 50, y: 50 }; 
      handleSelectFlower(visualMem);
  };
  ...
  const handleSpiritClick = () => {
      if (!hasWoken && hasStarted) {
          wakeSpirit();
      }
  };

  return (
    <main className="relative flex flex-col items-center justify-center w-full h-screen overflow-hidden bg-black touch-none" onMouseMove={(e) => { /* useParallax 내부에서 처리하므로 비움 */ }} onPointerDown={handleGlobalClick}>
      ...
      <motion.div className="absolute inset-0 w-full h-full" animate={{ filter: hasStarted ? 'blur(0px)' : 'blur(20px)', opacity: hasStarted ? 1 : 0 }} transition={{ duration: 2 }}>
        ...
        <motion.div 
            className="absolute inset-0 z-10 pointer-events-none bg-gradient-to-t from-black/80 via-gray-900/50 to-transparent mix-blend-hard-light"
            animate={{ opacity: hasWoken ? 0 : 1 }}
        ...
        {/* Spirit Animation */}
        <motion.div 
            className={`absolute inset-0 flex items-center justify-center ${!hasWoken ? 'cursor-pointer z-30' : 'pointer-events-none'}`} 
        ...
        >
           <motion.div 
               ...
               style={{ scale: isBreathing ? 1 : spiritScale }} 
               animate={ 
                   isSilentMode
                   ? { scale: 1.15, y: 20, filter: 'brightness(1.1) drop-shadow(0 0 30px rgba(255,255,255,0.4))' }
                   : !hasWoken 
                   ? { scale: 0.95, filter: 'brightness(0.4) grayscale(0.8) blur(2px)', y: [0, 5, 0] }
                   : isBreathing 
                   ? { scale: [1, 1.15, 1.15, 1], filter: ['brightness(1)', 'brightness(1.3) drop-shadow(0 0 30px rgba(255,255,255,0.6))', 'brightness(1.3) drop-shadow(0 0 30px rgba(255,255,255,0.6))', 'brightness(1)'] } 
                   : callStatus === 'idle' 
                   ? { y: [0, -15, 0], filter: ['brightness(0.8) drop-shadow(0 0 10px rgba(255,255,255,0.1))', 'brightness(1) drop-shadow(0 0 25px rgba(255,255,255,0.3))', 'brightness(0.8) drop-shadow(0 0 10px rgba(255,255,255,0.1))'] }
                   : callStatus === 'speaking' ? { y: 0, filter: 'brightness(1.2) drop-shadow(0 0 40px rgba(255,255,255,0.5))' } 
                   : { scale: 1.05, y: 0, filter: 'brightness(1.1)' } 
               } 
               transition={ 
                   !hasWoken 
                   ? { duration: 8, repeat: Infinity, ease: "easeInOut" } 
                   : isBreathing ? { duration: 19, times: [0, 0.21, 0.58, 1], repeat: Infinity, ease: "easeInOut" } 
                   : { y: { duration: 6, repeat: Infinity, ease: "easeInOut" }, filter: { duration: 4, repeat: Infinity, ease: "easeInOut", repeatType: "reverse" } } 
               }
           >
            <div className="absolute inset-0 z-20 pointer-events-none" style={{ backdropFilter: motionValues.blurValue as any, opacity: motionValues.opacityValue as any }} />
        ...
        {/* Bottom Right Buttons (Settings & Altar) */}
